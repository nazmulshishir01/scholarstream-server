...
router.get('/', async (req, res) => {
  try {
    const { search, category, country, degree, sort, page = 1, limit = 9 } = req.query;
    const { scholarships } = await getCollections();  // ✅ await
    let query = {};
    ...
    let sortOption = {};
    switch (sort) {
      case 'fees-asc': sortOption = { applicationFees: 1 }; break;
      case 'fees-desc': sortOption = { applicationFees: -1 }; break;
      case 'date-asc': sortOption = { scholarshipPostDate: 1 }; break;
      default: sortOption = { scholarshipPostDate: -1 };
    }
    ...
    const result = await scholarships.find(query).sort(sortOption).skip(skip).limit(limitNum).toArray();
...
router.get('/top', async (req, res) => {
  try {
    const { scholarships } = await getCollections();  // ✅ await
    const result = await scholarships.find({}).sort({ applicationFees: 1 }).limit(6).toArray();
    res.json(result);
  } catch (error) {
...
router.get('/related/:category/:id', async (req, res) => {
  try {
    ...
    const { scholarships } = await getCollections();  // ✅ await
    const result = await scholarships.find({
